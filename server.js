...
// Configuração segura para conexão com o banco (Neon / Supabase)
const pool = new Pool({ 
...
app.post('/api/sales', async (req, res) => {
  ...
  try {
    ...
    // Se vendeu menos unidades do que o lote total, divide o lote mantendo o resto em estoque
    if (qtySold < itemQty) {
...
app.delete('/api/sales/:id', async (req, res) => {
  ...
  try {
    ...
    await client.query("UPDATE items SET status='estoque' WHERE id=$1", [sale.rows[0].item_id]);
...
app.put('/api/loans/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [client_name, amount, repay, num_installments, loan_date, description || null, source_account || null, req.params.id]
    );
    res.json(result.rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// ─── FINANCIAL OVERVIEW (VISÃO GERAL QUEM DEVE) ──────────────────────────────
...
// ─── SALES HISTORY (HISTÓRICO ACORDEÃO POR MÊS) ──────────────────────────────
...
// ─── ACCOUNT BALANCE (DETALHAMENTO CONTA E DÉFICIT DO ESTOQUE) ────────────────
app.get('/api/account-balance', async (req, res) => {
  try {
    ...
    // INCLUSÃO SOLICITADA: Busca itens em estoque e em transporte para constar no rombo das contas
    ...
    const result = Object.values(accounts).filter(a => a.pending_reposto > 0.01 || a.total_invested > 0);
